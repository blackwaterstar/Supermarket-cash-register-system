...
public class ModifyCommodity extends JDialog{
	...
	public ModifyCommodity(JFrame owner, String title, boolean modal){
		...
		modify_Button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				ModifyComodityFrame mcFrame = new ModifyComodityFrame(jd, "修改商品信息", true,rowNum,commodityModel);
				String sql = CreateSql.getCommodity_Sql(null, "全部");//查询全部内容
				commodityModel = new CommodityModel(sql,jd);//构建新的数据模型类，并更新
				jt.setModel(commodityModel);
			}
